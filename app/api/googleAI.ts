  ...
  export async function getInformationFromGemini(input: Input | undefined, imagePart?:ImagePart) {
    ...
    const textPart = {
      text:
        ...
        The input object that you receive, is a JSON object, it provides more context about the user, things like age, location and language. You should use these information to tailor your response\
        The output data, including all fields, should be translated to whatever you receive under `input.language`, if nothing provided default is `en-US`. These ISO language codes. \
        For the multi language support make sure the JSON response you send back should follow the definition under Output and it should count for special charchters. \
        The output format will be a JSON object, withe the following definition in typescript which you use to respond to the prompts im this format only. You need to make sure it is a VALID JSON object and it supports special charchters from other languages like french in response.\n\ninterface Output {\n   art_title: string, // the title of the art work\n   artist_name: string, // the name of the artist\n   date: string, // date the art is created\n   more_about_artist: string, // a brief about artist, to show as complimentary data\n   brief_history: string, // a brief history, importance and why it's famous, what's important about it\n   technical_details: string, // from art point of view, what are some details? What details the audience should pay attention to, and why?\n  other_facts: string, // any fun cats, social or historical facts about it\n originalImageURL: string //simply returns the original image url submitted by user\n  recommended: {\n    art_title: string\n    artist_name: string,\n    date: string,\n\timage_url?:string // a thumbnail image, in formats jpg, png, jpeg\n   link: string // an external url to learn more about it\n   }[]\n} \n" + `input: ${JSON.stringify(input)}`,
    };
    parts.push(textPart);
