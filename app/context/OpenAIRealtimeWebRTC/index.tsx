...
const OpenAIRealtimeWebRTCContext = createContext<OpenAIRealtimeWebRTCContextType | undefined>(
  undefined,
);
...
export const useSession = (): OpenAIRealtimeWebRTCContextType => {
  ...
  if (!context) {
    throw new Error('useSession must be used within an OpenAIRealtimeWebRTCProvider');
  }
  return context;
};
...
export const OpenAIRealtimeWebRTCProvider: React.FC<OpenAIRealtimeWebRTCProviderProps> = ({
  config,
  children,
}) => {
  // Validate required config
  if (!config.realtimeApiUrl) {
    throw new Error('realtimeApiUrl is required in OpenAIRealtimeContextConfig');
  }
  if (!config.modelId) {
  ...
  const mergedConfig = {
    ...
    defaultAudioSettings: config.defaultAudioSettings ?? defaultConfig.defaultAudioSettings,
  };
  ...
  const connect: Connect = async (
    ...
    try {
      ...
      const audioSettings = realtimeSession.audioSettings ?? mergedConfig.defaultAudioSettings;

      // Get user media if audio modality is required and we have audio settings
      let localStream: MediaStream | undefined;
      if (realtimeSession.modalities?.includes(Modality.AUDIO) && audioSettings) {
        try {
      ...
      const attemptReconnection = async (pc: RTCPeerConnection): Promise<void> => {
        ...
        pc.oniceconnectionstatechange = () => {
          ...
          switch (state) {
            ...
            case 'disconnected':
              ...
              logger.warn(`Session '${sessionId}' disconnected. Attempting reconnection...`);
  ...
  const sendClientEvent = (event: RealtimeEvent): void => {
    ...
    if (!dataChannel || dataChannel.readyState !== 'open') {
      logger.error(`Data channel for session '${sessionId}' is not open. Cannot send event.`);
      return;
    }
    ...
    try {
      ...
      switch (event.type) {
        ...
        case RealtimeEventType.RATE_LIMITS_UPDATED: {
          ...
          const resetTime = new Date(Date.now() + maxResetSeconds * 1000).toISOString();
          const isRateLimited = rateLimitsEvent.rate_limits.some((limit) => limit.remaining <= 0);
